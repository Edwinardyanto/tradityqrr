...
### Admin Dashboard layout (post-2026-04-27 restructure)
...
4. **Funnel Traffic + reserved placeholder** — 1:1 row; left is a stacked bar (Kunjungan → Checkout → Form Diisi → Paid), right is an empty `cc-panel-empty` ("Coming soon") reserved for future content
5. **Recent orders + Top affiliates** — live SSE updates, 30d affiliate leaderboard
...
- [ ] Reserved placeholder card on Dashboard (right of Funnel Traffic) — empty "Coming soon" cc-panel waiting for content
